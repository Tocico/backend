...
<body>
  <?php
    ...
    if (isset($_GET['model']) && isset($_GET['type'])){
      echo $_GET['model'];
      echo $_GET['type'];
    $statement = $pdo->prepare("DELETE FROM {$_GET['type']}  WHERE model = {$_GET['model']}"); 
      $statement->execute();
    $statement = $pdo->prepare("DELETE FROM product WHERE model = {$_GET['model']}"); 
      $statement->execute();
    }
  ...
  <table>
    ...
    ?>
      <tr>
        ...
        <td><a href="?model=<?= $product['model'] ?>&type=<?= $product['type'] ?>">Delete</td>
      </tr>
    <?php
      }
    ?>
